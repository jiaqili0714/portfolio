...
To use a .bib file for citations, we need to add `bibliography: FILE_NAME.bib` to our YAML header.  Then, where we want an in-text citation, we need to say @KEY.  The Key is the very first string of characters following the opening bracket at the top of a reference.  For example, @xie2020r is the key for the R Markdown Cookbook. When you knit the document with the YAML header, there will be an in-text citation in the key's place.  The full citation will also appear at the bottom of the document. 
