...
> TO DO, FINISH GITHUB SECTION, ADD SECTION ABOUT BIBLOGRAPHY ALONG WITH A .bib FILE WHICH THEYRE GOING TO EDIT THEMSELVES
...
To add citations to your R Markdowns, you can either manually type add them, or you can add a BibTex file (.bib) and use that to automatically insert citations.  BibTex files can be exported from citation managers like Zotero, or you can even use cites like Google Scholar to retrieve a citation in BibTex formatting then paste it into your BibTex file. Empty .bib's have already been provided in your Mini Project folders.  Open the `setting-up.bib` file by clicking it in the Files menu on the bottom right.  There should be one reference in it already

To use a .bib file for citations, we need to add `bibliography: FILE_NAME.bib` to our YAML header.  Then, where we want an in-text citation, we need to say @KEY.  The Key is the very first string of characters following the opening bracket at the top of a reference.  For example, @xie2020r . When you knit the document, that will be an in-text citation.

> Checkpoing 4: Go to google scholar, search for "R Markdown: The Definitive Guide", copy the BibTex citation and paste it into setting-up.bib.  Then, add an in-text citation below. Knit your document to make sure it shows up.  
...
> Checkpoint 7: Replace "YOUR DATASET NAME" with the name of your cars dataset.  Remove the #'s. What did the mutate function do?  Hint: If you're unsure what a function does, typing ?Function into the Console at the bottom of the screen will pull up its Help page.  
...
> Checkpoint 8: Make a code chunk below.  With the Tidyverse command `filter`, find only the cars that took longer than 20 meters to stop.  There should be 10.  (Use ?filter and scroll to the bottom to see examples if you need. Note that the examples all say the dataset as the first item, but because of piping, you don't need to).  
...
> Checkpoint 9: If you would like additional help with R, replace this text.  
