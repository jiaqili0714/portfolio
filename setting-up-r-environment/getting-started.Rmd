...
To add citations to R Markdowns, we can either manually type add them, or we can add a BibTex file (.bib) and use that to automatically insert citations.  BibTex files can be exported from citation managers like Zotero, or we can even use websites like Google Scholar to retrieve a citation in BibTex formatting then paste it into your BibTex file. Empty .bib's have already been provided in your Mini Project folders.  Open the `setting-up.bib` file by clicking it in the Files menu on the bottom right.  There should be one reference in it already

To use a .bib file for citations, we need to add `bibliography: FILE_NAME.bib` to our YAML header.  Then, where we want an in-text citation, we need to say @KEY.  The Key is the very first string of characters following the opening bracket at the top of a reference.  For example, @xie2020r . When you knit the document, there will be an in-text citation in its place.
...
> Checkpoint 9: Make a code chunk below.  With the Tidyverse command `filter`, find only the cars that took longer than 20 meters to stop.  There should be 10.  (Use ?filter and scroll to the bottom to see examples if you need. Note that the examples all say the dataset as the first item, but because of piping, you don't need to).  
...
Add something like this: this markdown file we anticipate would be largely review for many of you.  If you're amongst the folks where you're less comfortable with the material, please let us know so we can best support you.

> Checkpoint 10: If you would like additional help with R, replace this text.  
